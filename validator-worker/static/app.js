// https://cdn.skypack.dev/-/lit-html@v1.4.1-JeRAopm3gMvLb2eFFSHK/dist=es2019,mode=imports/optimized/lit-html.js
...
// https://cdn.skypack.dev/-/lit-element@v2.5.1-A63g9TYe4wXjHnI3CPWK/dist=es2019,mode=imports/optimized/lit-element.js
var _a;
...
var UpdatingElement = class extends HTMLElement {
  ...
  static finalize() {
    ...
    if (this.hasOwnProperty(JSCompiler_renameProperty("properties", this))) {
      ...
      const propKeys = [
        ...Object.getOwnPropertyNames(props2),
        ...typeof Object.getOwnPropertySymbols === "function" ? Object.getOwnPropertySymbols(props2) : []
      ];
      for (const p of propKeys) {
        this.createProperty(p, props2[p]);
      }
    }
  }
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/theme.ts
var Theme = class {
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/qnames.ts
var PODCAST_INDEX_NAMESPACE = "https://podcastindex.org/namespace/1.0";
var PODCAST_INDEX_NAMESPACE_ALT = "https://github.com/Podcastindex-org/podcast-namespace/blob/main/docs/1.0.md";
var PODCAST_INDEX_NAMESPACE_KNOWN_MISSPELLING = "https://podcastindex.org/namespace/1.0/";
var PODCAST_INDEX_NAMESPACES = [PODCAST_INDEX_NAMESPACE, PODCAST_INDEX_NAMESPACE_ALT, PODCAST_INDEX_NAMESPACE_KNOWN_MISSPELLING];
var PODCAST_INDEX_KNOWN_NAMES = /* @__PURE__ */ new Set();
...
var MEDIA_RSS_NAMESPACE = "http://search.yahoo.com/mrss/";
function _mediaRss(name) {
  return { name, namespaceUri: MEDIA_RSS_NAMESPACE };
}
var ITUNES_NAMESPACE = "http://www.itunes.com/dtds/podcast-1.0.dtd";
...
var Qnames = class {
  static PodcastIndex = {
    ...
    KNOWN_MISSPELLED_NAMESPACES: [PODCAST_INDEX_NAMESPACE_KNOWN_MISSPELLING],
    get KNOWN_NAMES() {
      return PODCAST_INDEX_KNOWN_NAMES;
    },
    of: (name) => _podcastIndex(
      name,
      false
      /*known*/
    ),
...
// https://raw.githubusercontent.com/skymethod/denoflare/fdc2300b204f5f5e6205dfb970b7f348383c304c/common/check.ts
function checkEqual(name, value, expected) {
  if (value !== expected) throw new Error(`Bad ${name}: expected ${expected}, found ${value}`);
}

// https://cdn.skypack.dev/-/strnum@v1.0.5-UZPHteCsFGtQtMgV7Cjq/dist=es2019,mode=imports/optimized/strnum.js
...
var strnum = toNumber;
var strnum_default = strnum;

// https://cdn.skypack.dev/-/fast-xml-parser@v3.21.1-GF6KBYcx54Lz1yeJBPtP/dist=es2019,mode=imports/optimized/fast-xml-parser.js
...
function parseValue(val, shouldParse, options) {
  if (shouldParse && typeof val === "string") {
    ...
    if (newval === "true")
      ...
      return strnum_default(val, options);
  } else {
    if (util.isExist(val)) {
      return val;
    } else {
...
var props$1 = ["allowBooleanAttributes"];
...
var buildOptions$2 = util.buildOptions;
...
var convertToJson$1 = parser.convertToJson;
var convertToJsonString$1 = parser.convertToJsonString;
var convertTonimn = parser.convertTonimn;
var getTraversalObj$1 = parser.getTraversalObj;
var j2xParser = parser.j2xParser;
var parse = parser.parse;
var parseToNimn = parser.parseToNimn;
var validate$1 = parser.validate;

// https://raw.githubusercontent.com/skymethod/denoflare/fdc2300b204f5f5e6205dfb970b7f348383c304c/common/xml_util.ts
function encodeXml(unencoded) {
  return unencoded.replaceAll(/[&<>'"]/g, (char2) => {
    return UNENCODED_CHARS_TO_ENTITIES[char2];
  });
}
...
var UNENCODED_CHARS_TO_ENTITIES = {
  "<": "&lt;",
  ">": "&gt;",
  "&": "&amp;",
  "'": "&#39;",
...
var ENTITIES_TO_UNENCODED_CHARS = {
  "lt": "<",
  "gt": ">",
  "amp": "&",
  "apos": `'`,
...
// https://raw.githubusercontent.com/skymethod/denoflare/fdc2300b204f5f5e6205dfb970b7f348383c304c/common/xml_parser.ts
...
function parseXml(xml, opts = {}) {
  ...
  const rt = getTraversalObj$1(xml, { ignoreAttributes: false, parseAttributeValue: false, parseNodeValue: false, tagValueProcessor });
...
var EMPTY_STRING_MAP = /* @__PURE__ */ new Map();
var EMPTY_XML_NODE_ARRAY = [];
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/check.ts
function checkMatches(name, value, pattern) {
  if (!pattern.test(value)) throw new Error(`Bad ${name}: ${value}`);
  return value;
}
function checkEqual2(name, value, expected) {
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/util.ts
function isReadonlyArray(arg) {
  return Array.isArray(arg);
}

// Users/js/data/repos-llc/livewire-validator/validator-worker/common/validation_functions.ts
...
function isUri(trimmedText) {
  return tryParseUrl(trimmedText) !== void 0;
}
...
function hasApplePodcastsSupportedFileExtension(url) {
  ...
  return u !== void 0 && /\.(m4a|mp3|mov|mp4|m4v|pdf)$/i.test(u.pathname);
}
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/validator.ts
...
function validateRss(rss, callbacks) {
  const opts = { reference: { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#whatIsRss" } };
  ...
  const itunesOpts = { reference: { ruleset: "itunes", href: "https://podcasters.apple.com/support/823-podcast-requirements#:~:text=Podcast%20RSS%20feed%20technical%20requirements" } };
  const hasItunesPrefix = findElementRecursive(rss, (v) => v.tagname.startsWith("itunes:")) !== void 0;
  if (hasItunesPrefix) checkAttributeEqual(rss, "xmlns:itunes", Qnames.Itunes.NAMESPACE, callbacks, itunesOpts);
  const hasContentPrefix = findElementRecursive(rss, (v) => v.tagname.startsWith("content:")) !== void 0;
...
function validateChannel(channel, callbacks) {
  const opts = { reference: { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#requiredChannelElements" } };
  ...
  const image = ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelImageReference, { name: "image" }).checkRemainingAttributes().node;
  if (image) {
    ElementValidation.forRequiredSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "url" }).checkValue(isUrl).checkRemainingAttributes();
    ElementValidation.forRequiredSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "title" }).checkValue(isNotEmpty).checkRemainingAttributes();
    ElementValidation.forRequiredSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "link" }).checkValue(isUrl).checkRemainingAttributes();
    ElementValidation.forSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "width" }).checkValue(isPositiveInteger).checkRemainingAttributes();
    ElementValidation.forSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "height" }).checkValue(isPositiveInteger).checkRemainingAttributes();
    ElementValidation.forSingleChild("channel image", image, callbacks, rssChannelImageReference, { name: "description" }).checkValue(isNotEmpty).checkRemainingAttributes();
  }
  const rssChannelOptional = { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#optionalChannelElements" };
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "language" }).checkValue(isRssLanguage).checkRemainingAttributes();
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "managingEditor" }).checkValue(isEmailAddressWithOptionalName).checkRemainingAttributes();
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "webMaster" }).checkValue(isEmailAddressWithOptionalName).checkRemainingAttributes();
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "pubDate" }).checkValue(isRfc2822).checkRemainingAttributes();
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "lastBuildDate" }).checkValue(isRfc2822).checkRemainingAttributes();
  for (const category of findChildElements(channel, { name: "category " })) {
    ElementValidation.forElement("channel category", category, callbacks, { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#ltcategorygtSubelementOfLtitemgt" }).checkValue(isNotEmpty).checkOptionalAttribute("domain", isNotEmpty).checkRemainingAttributes();
  }
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "docs" }).checkValue(isUrl).checkRemainingAttributes();
  ElementValidation.forSingleChild("channel", channel, callbacks, rssChannelOptional, { name: "ttl" }).checkValue(isNonNegativeInteger).checkRemainingAttributes();
  ...
  for (const item of items) {
    ...
    const elements = findChildElements(item, { name: "enclosure" });
  ...
  for (const valueElement of findChildElements(node, ...Qnames.PodcastIndex.value)) {
    ...
    if (value) {
      ...
      for (const valueTimeSplit of findChildElements(value, ...Qnames.PodcastIndex.valueTimeSplit)) {
        ...
        if (!validValue) callbacks.onWarning(valueElement, `Bad <${valueElement.tagname}> <podcast:valueTimeSplit> node value: expected a single <podcast:remoteItem> element OR one or more <podcast:valueRecipient> elements.`, { reference: valueTimeSplitReference });
...
function checkPodcastTxt(level, node, callbacks) {
  ...
  for (const txt of txts) {
    ElementValidation.forElement(level, txt, callbacks, txtReference).checkOptionalAttribute("purpose", isAtMostCharacters(128)).checkValue(isAtMostCharacters(4e3)).checkRemainingAttributes();
  }
}
...
function checkPodcastTagUsage(node, callbacks) {
  ...
  const namespaceUris = /* @__PURE__ */ new Set();
...
var itunesPodcastersGuide = { ruleset: "itunes", href: "https://help.apple.com/itc/podcasts_connect/#/itcb54353390" };
function validateItem(item, callbacks, itemTagName) {
  const itunesOpts1 = { reference: { ruleset: "itunes", href: "https://podcasters.apple.com/support/823-podcast-requirements#:~:text=Podcast%20RSS%20feed%20technical%20requirements" } };
  const itunesOpts2 = { reference: itunesPodcastersGuide };
  ...
  if (enclosure) {
    const rssEnclosureOpts = { reference: { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#ltenclosuregtSubelementOfLtitemgt" } };
  ...
  if (guid) {
    ...
    const rssGuidOpts = { reference: { ruleset: "rss", href: "https://cyber.harvard.edu/rss/rss.html#ltguidgtSubelementOfLtitemgt" } };
  ...
  ElementValidation.forSingleChild(itemTagName, item, callbacks, podcastIndexReference("https://github.com/Podcastindex-org/podcast-namespace/blob/main/docs/1.0.md#season"), ...Qnames.PodcastIndex.season).checkOptionalAttribute("name", (v) => isNotEmpty(v) && isAtMostCharacters(128)(v)).checkValue(isNonNegativeInteger).checkRemainingAttributes();
  ElementValidation.forSingleChild(itemTagName, item, callbacks, podcastIndexReference("https://github.com/Podcastindex-org/podcast-namespace/blob/main/docs/1.0.md#episode"), ...Qnames.PodcastIndex.episode).checkOptionalAttribute("display", (v) => isNotEmpty(v) && isAtMostCharacters(32)(v)).checkValue(isDecimal).checkRemainingAttributes();
...
var ElementValidation = class _ElementValidation {
  static EMPTY_STRING_SET = /* @__PURE__ */ new Set();
  node;
  level;
  callbacks;
  opts;
  ...
  constructor(level, node, callbacks, opts) {
    ...
    this.opts = opts;
    this.remainingAttNames = node ? new Set(node.atts.keys()) : _ElementValidation.EMPTY_STRING_SET;
  }
  static forElement(level, node, callbacks, reference) {
    return new _ElementValidation(level, node, callbacks, { reference });
  }
  static forRequiredSingleChild(level, parent, callbacks, reference, qname) {
    const elements = findChildElements(parent, qname);
    if (elements.length === 1) {
      return new _ElementValidation(level, elements[0], callbacks, { reference });
    }
    if (elements.length === 0) {
    ...
    return new _ElementValidation(level, void 0, callbacks, { reference });
  }
  static forSingleChild(level, parent, callbacks, reference, ...qnames) {
    ...
    if (elements.length > 0) {
      ...
      return new _ElementValidation(level, element, callbacks, { reference });
    }
    return new _ElementValidation(level, void 0, callbacks, { reference });
  }
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/sets.ts
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/check.ts
...
// https://raw.githubusercontent.com/skymethod/denoflare/96ad1ceeb97f6506b394c71332fe99bd72d1d073/common/bytes.ts
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/crypto.ts
var DEFAULT_ALLOWED_SECONDS_IN_THE_PAST = 60 * 60 * 12;
var DEFAULT_ALLOWED_SECONDS_IN_THE_FUTURE = 60 * 60;

// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap_implementation.ts
...
function destructureThreadcapUrl(url) {
  ...
  const tmpUrl2 = m2 ? `https://${m2[2]}` : void 0;
...
async function findOrFetchTextResponse(url, after, fetcher, cache, opts) {
  ...
  await cache.put(url, (/* @__PURE__ */ new Date()).toISOString(), response);
  return response;
}
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap_activitypub.ts
...
async function mastodonFindReplies(id, opts) {
  ...
  if (debug) console.log(JSON.stringify(obj, void 0, 2));
  const rt = [];
  if (isStringRecord2(obj) && Array.isArray(obj.descendants)) {
    for (const descendant of obj.descendants) {
      if (isStringRecord2(descendant) && typeof descendant.uri === "string" && descendant.in_reply_to_id === statusId) {
...
async function initActivityPubThreadcap(url, opts) {
  ...
  const object = await findOrFetchActivityPubObject(url, (/* @__PURE__ */ new Date()).toISOString(), fetcher, cache);
...
function unwrapActivityIfNecessary(object, id, callbacks) {
  if (object.type === "Create" && isStringRecord2(object.object)) {
...
function computeUrl(url) {
  if (url === void 0 || url === null) return void 0;
    ...
    for (const prop of ["oneOf", "anyOf"]) {
      ...
      if (Array.isArray(val)) {
        for (const item of val) {
          if (isStringRecord2(item) && item.type === "Note" && typeof item.name === "string") {
            if (!rt) rt = [];
            rt.push(item.name);
          } else {
...
function computeAttributedTo(attributedTo) {
  ...
  if (Array.isArray(attributedTo) && attributedTo.length > 0) {
    ...
    if (attributedTo.every((v) => isStringRecord2(v))) {
...
function computeLanguageTaggedValues(obj, stringProp, mapProp) {
  if (obj.type === "PodcastEpisode" && isStringRecord2(obj.description) && obj.description.type === "Note") obj = obj.description;
  const stringVal = obj[stringProp] ?? void 0;
  const mapVal = obj[mapProp] ?? void 0;
  if (stringVal !== void 0 && typeof stringVal !== "string") throw new Error(`Expected '${stringProp}' to be a string, found ${JSON.stringify(stringVal)}`);
  if (mapVal !== void 0 && !(isStringRecord2(mapVal) && Object.values(mapVal).every((v) => typeof v === "string"))) throw new Error(`Expected '${mapProp}' to be a string record, found ${JSON.stringify(mapVal)}`);
  if (mapVal !== void 0) return mapVal;
  if (stringVal !== void 0) return { und: stringVal };
  if (obj.type === "Video" && typeof obj.name === "string" && isNonEmpty(obj.name)) return { und: obj.name };
}
function computeAttachments(object) {
  const rt = [];
  if (!object.attachment) return rt;
  const attachments = isReadonlyArray2(object.attachment) ? object.attachment : [object.attachment];
...
function computeAttachment(object) {
  ...
  if (width !== void 0 && typeof width !== "number") throw new Error(`Expected attachment 'width' to be a number, found ${JSON.stringify(width)}`);
  if (height !== void 0 && typeof height !== "number") throw new Error(`Expected attachment 'height' to be a number, found ${JSON.stringify(height)}`);
...
function computeCommenter(person, asof) {
  let icon2;
  if (person.icon) {
    if (typeof person.icon !== "object" || isReadonlyArray2(person.icon) || person.icon.type !== "Image") throw new Error(`Expected person 'icon' to be an object, found: ${JSON.stringify(person.icon)}`);
    icon2 = computeIcon(person.icon);
  }
  const { name, preferredUsername, url: apUrl, id } = person;
  if (name !== void 0 && typeof name !== "string") throw new Error(`Expected person 'name' to be a string, found: ${JSON.stringify(person)}`);
  if (preferredUsername !== void 0 && typeof preferredUsername !== "string") throw new Error(`Expected person 'preferredUsername' to be a string, found: ${JSON.stringify(person)}`);
  ...
  if (apUrl !== void 0 && typeof apUrl !== "string") throw new Error(`Expected person 'url' to be a string, found: ${JSON.stringify(apUrl)}`);
  const url = apUrl || id;
  ...
  return { icon: icon2, name: nameOrPreferredUsername, url, fqUsername, asof };
}
function computeIcon(image) {
  ...
  if (mediaType !== void 0 && typeof mediaType !== "string") throw new Error(`Expected icon 'mediaType' to be a string, found: ${JSON.stringify(mediaType)}`);
...
async function mastodonFindStatusIdForActivityPubId(id, after, fetcher, cache, debug) {
  ...
  const obj = await findOrFetchJson(url.toString(), after, fetcher, cache, { accept: "application/json" });
  if (debug) console.log(JSON.stringify(obj, void 0, 2));
  if (isStringRecord2(obj) && Array.isArray(obj.statuses) && obj.statuses.length === 1) {
    const status = obj.statuses[0];
    if (isStringRecord2(status) && typeof status.id === "string" && status.id !== "") {
      return status.id;
    }
  }
  return void 0;
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap_bluesky.ts
var BlueskyProtocolImplementation = {
  async initThreadcap(url, opts) {
    const { uri, nodes, commenters } = await getThread(
      url,
      opts,
      1e3
      /* max */
...
function isGetPostThreadResponse(obj) {
  return isStringRecord2(obj) && isThreadViewPost(obj.thread);
}
function isThreadViewPost(obj) {
  return isStringRecord2(obj) && obj["$type"] === "app.bsky.feed.defs#threadViewPost" && isStringRecord2(obj.post) && typeof obj.post.uri === "string" && isStringRecord2(obj.post.author) && typeof obj.post.author.did === "string" && typeof obj.post.author.handle === "string" && (obj.post.author.displayName === void 0 || typeof obj.post.author.displayName === "string") && (obj.post.author.avatar === void 0 || typeof obj.post.author.avatar === "string") && Array.isArray(obj.post.author.labels) && isStringRecord2(obj.post.record) && obj.post.record["$type"] === "app.bsky.feed.post" && typeof obj.post.record.text === "string" && (obj.post.replyCount === void 0 || typeof obj.post.replyCount === "number") && (obj.replies === void 0 || Array.isArray(obj.replies) && obj.replies.every(isThreadViewPost));
}
function isGetProfileResponse(obj) {
  return isStringRecord2(obj) && typeof obj.did === "string" && typeof obj.handle === "string" && typeof obj.displayName === "string" && (obj.avatar === void 0 || typeof obj.avatar === "string");
}
...
async function getProfile(handleOrDid, { updateTime, fetcher, cache, bearerToken }) {
  ...
  if (!isGetProfileResponse(res)) throw new Error(JSON.stringify(res, void 0, 2));
  return res;
}
async function getThread(url, opts, depth) {
  const { debug, fetcher, updateTime = (/* @__PURE__ */ new Date()).toISOString(), cache, bearerToken } = opts;
  ...
  if (!isGetPostThreadResponse(res)) throw new Error(`Expected GetPostThreadResponse: ${JSON.stringify(res, void 0, 2)}`);
  if (debug) console.log(JSON.stringify(res, void 0, 2));
  const nodes = {};
  const commenters = {};
  const processThread = (thread) => {
    ...
    if (replyCount === void 0) {
      if (thread.replies !== void 0) throw new Error(`Expected no thread.replies for undefined replyCount`);
    } else {
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap_nostr.ts
var NostrProtocolImplementation = {
  async initThreadcap(url, opts) {
    const { debug, updateTime = (/* @__PURE__ */ new Date()).toISOString() } = opts;
    ...
    const makeUri = ({ message }) => {
      ...
      if (message !== void 0) u.searchParams.set("message", message);
      return u.toString();
    };
    const state = {};
    ...
    for (const message of messages) {
      ...
      nodes[uri] = {
        comment: {
          ...
          published: new Date(message.created_at * 1e3).toISOString()
        },
        commentAsof: updateTime,
        replies: [],
        repliesAsof: updateTime
    ...
    const activities = await query({
      kinds: [30311],
      limit: 1e3,
      tags: dumpActivities ? void 0 : {
        "#d": [identifier]
      }
    }, { hostname, debug, state });
    if (dumpActivities) console.log(activities.map((v) => `activity: ${v.tags.find((v2) => v2[0] === "d")?.at(1)}	${v.tags.find((v2) => v2[0] === "title")?.at(1)}`).join("\n"));
    const activity = activities.find((v) => v.tags.some((v2) => v2[0] === "d" && v2[1] === identifier));
    const rootUri = makeUri({ message: void 0 });
    nodes[rootUri] = {
      comment: {
        attachments: [],
        attributedTo: activity ? activity.pubkey : "",
        content: { un: activity?.tags.find((v) => v[0] === "title")?.at(1) ?? "" },
        published: activity ? new Date(activity.created_at * 1e3).toISOString() : void 0
      },
      commentAsof: updateTime,
    ...
    for (const pubkey of pubkeys) {
      ...
      if (profile) {
        ...
        commenters[pubkey] = {
          ...
          fqUsername: lud16,
          icon: picture ? { url: picture } : void 0,
          url: website,
          asof: updateTime
        };
      } else {
...
async function query(filter, opts) {
  ...
  const ws = await (async () => {
    ...
    if (existing) return existing;
    const { resolve: resolve2, reject: reject2, promise: promise2 } = promiseWithResolvers();
    const ws2 = new WebSocket(`wss://${hostname}`);
    state[stateKey] = ws2;
    ws2.onopen = () => {
      if (debug) console.log("onopen");
      resolve2(void 0);
    };
    ws2.addEventListener("error", () => reject2());
    ws2.addEventListener("close", () => reject2());
    await promise2;
    return ws2;
  })();
  ...
  ws.addEventListener("message", ({ data }) => {
    if (done()) return;
    if (debug) console.log(`onmessage: ${typeof data === "string" && data.startsWith("[") && data.endsWith("]") ? JSON.stringify(JSON.parse(data), void 0, 2) : JSON.stringify(data)}`);
    let parsed;
    try {
...
// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap_twitter.ts
var TwitterProtocolImplementation = {
  async initThreadcap(url, opts) {
    ...
    const obj = await findOrFetchTwitter(tweetApiUrl, (/* @__PURE__ */ new Date()).toISOString(), opts);
    if (debug) console.log(JSON.stringify(obj, void 0, 2));
  ...
  async fetchCommenter(attributedTo, opts) {
    ...
    const obj = await findOrFetchTwitter(url.toString(), updateTime, opts);
    if (debug) console.log("fetchCommenter", JSON.stringify(obj, void 0, 2));
...
function computeCommenterFromUserObj(obj, asof) {
  ...
  const icon2 = iconUrl ? { url: iconUrl, mediaType: iconMediaType } : void 0;
  return {
    asof,
    name,
    fqUsername,
...
async function findOrFetchConversation(tweetId, opts) {
  ...
  if (!conversation) {
    ...
    while (++i) {
      ...
      const obj = await findOrFetchTwitter(url.toString(), updateTime, opts);
      if (debug) console.log(`findOrFetchConversation nextToken=${nextToken}`, JSON.stringify(obj, void 0, 2));
...
async function findOrFetchConversationId(tweetId, opts) {
  ...
  const obj = await findOrFetchTwitter(url.toString(), updateTime, opts);
  if (debug) console.log("findOrFetchConversation", JSON.stringify(obj, void 0, 2));
  conversationId = obj.data.conversation_id;
  if (typeof conversationId !== "string") throw new Error(`Unexpected conversationId in payload: ${JSON.stringify(obj, void 0, 2)}`);
  state.conversationId = conversationId;
  return conversationId;
}

// https://raw.githubusercontent.com/skymethod/minipub/027cc569cb0ee31bc8440ea582fab365f9fe212a/src/threadcap/threadcap.ts
var MAX_LEVELS = 1e3;
...
function makeRateLimitedFetcher(fetcher, opts = {}) {
  ...
  return async (url, opts2) => {
    ...
    if (limits) {
      ...
      if (millisToWait > 0) {
        callbacks?.onEvent({ kind: "waiting-for-rate-limit", endpoint, millisToWait, millisTillReset, limit: limit2, remaining: remaining2, reset: reset2 });
...
function computeProtocolImplementation(protocol) {
  if (protocol === void 0 || protocol === "activitypub") return ActivityPubProtocolImplementation;
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/validation_job_vm.ts
var ValidationJobVM = class {
  fetchers;
  piSearchFetcher;
  threadcapUserAgent;
  nextJobId = 1;
  ...
  //
  get validating() {
    return this.currentJob !== void 0 && !this.currentJob.done;
  }
  get done() {
    return this.currentJob !== void 0 && this.currentJob.done;
  }
  ...
  get isSearch() {
    return this.currentJob !== void 0 && this.currentJob.search;
  }
  get searchResults() {
  ...
  constructor(opts) {
    const { localFetcher: localFetcher2, remoteFetcher: remoteFetcher2, piSearchFetcher: piSearchFetcher2, threadcapUserAgent: threadcapUserAgent2 } = opts;
    this.fetchers = { localFetcher: localFetcher2, remoteFetcher: remoteFetcher2 };
    this.piSearchFetcher = piSearchFetcher2;
    this.threadcapUserAgent = threadcapUserAgent2;
  }
  onChange = () => {
  };
  continueWith(url) {
  ...
  async validateAsync(input, job) {
    ...
    try {
      ...
      if (input.startsWith("https://t.co/")) {
        ...
        const { response } = await localOrRemoteFetch(input, { fetchers, headers: tcoHeaders, useSide: "remote" });
        if (job.done) return;
        ...
        const inputUrl = tryParseUrl2(input);
        ...
        if (inputUrl.protocol === "nostr:") {
          ...
          const { response: r, side, fetchTime } = await localOrRemoteFetch(inputUrl.toString(), { fetchers, headers });
          if (job.done) return;
          ...
          checkEqual2(`${inputUrl.host} response status`, r.status, 200);
          contentType = r.headers.get("Content-Type") ?? void 0;
          response = r;
        }
        ...
        if (response && contentType && contentType.includes("/html")) {
          if (inputUrl.hostname.endsWith("twitter.com")) {
            ...
            validateFeed = false;
            bluesky = { url: input, subject: "input url" };
          } else {
            ...
            if (inputUrl.hostname.endsWith("threads.net")) {
              ...
              const apUrl = /<link rel="alternate" href="(https:\/\/(www\.)?threads\.net\/ap\/[^"]+)" type="application\/activity\+json" \/>/.exec(html2)?.at(1);
        ...
        if (response && validateFeed) {
          ...
          if (xml) {
              ...
              if (opts?.tag === "social-interact") {
                ...
                if (uri) {
                  ...
                  if (attributes.get("protocol")?.toLowerCase() === "bluesky") {
                    const episodeTitle = findEpisodeTitle(node);
                    bluesky = { url: uri, subject: episodeTitle ? `\u201C${episodeTitle}\u201D` : "episode" };
                  }
                }
              }
            };
            const knownPiTags = /* @__PURE__ */ new Set();
            const unknownPiTags = /* @__PURE__ */ new Set();
            const piNamespaceUris = /* @__PURE__ */ new Set();
            let rssItemInfo;
            ...
            const tagString = (set) => [...set].sort().map((v) => `<podcast:${v}>`).join(", ");
        ...
        const validateComments = job.options.validateComments !== void 0 ? job.options.validateComments : true;
        if (hasComments && !validateComments) {
          ...
          for (const activityPub of activityPubs) {
            ...
            const remoteOnlyOrigins = /* @__PURE__ */ new Set();
            const computeUseSide = (url) => {
              return remoteOnlyOrigins.has(new URL(url).origin) ? "remote" : void 0;
            };
            let activityPubCalls = 0;
            const fetchActivityPubOrMastodon = async (url, opts) => {
              const { headers: headers2 } = opts || {};
              const localOrRemoteFetchFunction = headers2 && headers2.accept === "application/json" ? localOrRemoteFetchJson : localOrRemoteFetchActivityPub;
              let { response: response2, side } = await localOrRemoteFetchFunction(url, fetchers, computeUseSide(url), sleepMillisBetweenCalls);
              let obj = await response2.clone().json();
              console.log(JSON.stringify(obj, void 0, 2));
              if (url.includes("/api/v1/statuses") && typeof obj.uri === "string") {
                url = obj.uri;
                const { response: response22, side: side2 } = await localOrRemoteFetchFunction(url, fetchers, computeUseSide(url), sleepMillisBetweenCalls);
                response2 = response22.clone();
                obj = await response22.json();
                side = side2;
                console.log(JSON.stringify(obj, void 0, 2));
              }
              if (side === "remote") {
          ...
          if (twitter) {
            ...
            const fetchTwitterComments = async (url) => {
              const { response: response2 } = await localOrRemoteFetchJson(url, fetchers, "remote", sleepMillisBetweenCalls);
          ...
          if (bluesky) {
            ...
            const fetcher = makeRateLimitedFetcher(fetchBlueskyComments, { callbacks });
            const cache = new InMemoryCache();
            const userAgent = this.threadcapUserAgent;
            const updateTime = (/* @__PURE__ */ new Date()).toISOString();
            const threadcap = await makeThreadcap(bluesky.url, { userAgent, fetcher, cache, updateTime, protocol: "bluesky" });
            job.commentsResults = [...results, { threadcap, subject: bluesky.subject }];
            this.onChange();
            await updateThreadcap(threadcap, { updateTime, keepGoing, userAgent, fetcher, cache, callbacks });
            job.times.commentsTime = Date.now() - start;
        ...
        checkEqual2("searchResponse.status", searchResponse.status, 200);
...
function formatTime(millis) {
  ...
  return `${Math.round(millis / 1e3 * 100) / 100}s`;
}
function computeJobTimesStringSuffix(times) {
  const rt = [["fetch", times.fetchTime], ["read", times.readTime], ["parse", times.parseTime], ["validate", times.validateTime], ["comments", times.commentsTime]].filter((v) => v[1] !== void 0).map((v) => `${v[0]}=${formatTime(v[1])}`).join(", ");
  return rt === "" ? "" : ` (${rt})`;
}
...
async function localOrRemoteFetchActivityPub(url, fetchers, useSide, sleepMillisBetweenCalls) {
  if (sleepMillisBetweenCalls > 0) await sleep2(sleepMillisBetweenCalls);
  const { response, side } = await localOrRemoteFetch(url, { fetchers, headers: { "Accept": "application/activity+json" }, useSide });
  checkEqual2("res.status", response.status, 200);
...
async function localOrRemoteFetch(url, opts) {
  ...
  if (useSide !== "remote") {
    try {
      ...
      const response2 = await fetchers.localFetcher(url, headers);
      if (response2.status === 429) throw new Error(`429: ${await response2.text()}`);
      return { fetchTime: Date.now() - start2, side: "local", response: response2 };
    } catch (e) {
...
// Users/js/data/repos-llc/livewire-validator/validator-worker/common/oauth.ts
...
async function fetchJson(req, bodyVerifier, { allow202, fetcher = fetch } = {}) {
  ...
  if (!bodyVerifier(body)) throw new Error(`Unexpected body: ${JSON.stringify(body, void 0, 2)}`);
  return body;
}

// Users/js/data/repos-llc/livewire-validator/validator-worker/common/mastodon_api.ts
...
function isStatus(obj) {
  return isStringRecord(obj) && typeof obj.id === "string" && typeof obj.uri === "string" && typeof obj.created_at === "string" && typeof obj.content === "string" && typeof obj.visibility === "string" && (obj.url === void 0 || typeof obj.url === "string") && (obj.in_reply_to_id === void 0 || obj.in_reply_to_id === null || typeof obj.in_reply_to_id === "string");
}

// Users/js/data/repos-llc/livewire-validator/validator-app/validator_app_vm.ts
var ValidatorAppVM = class {
  job;
  //
  get validating() {
  ...
  isLoggedIn(origin) {
    ...
    return info !== void 0 && !computeExpired(info.tokenResponse);
  }
  acceptLogin(origin, tokenResponse) {
    checkEqual2("token_type", tokenResponse.token_type.toLowerCase(), "bearer");
  ...
  async sendReply(reply, replyToUrl) {
    ...
    const mastodonId = await computeMastodonIdForUrl(replyToUrl, async (url2, headers) => {
      const { response } = await this.job.fetch(url2, { headers });
      return response;
    });
...
function computeExpired(tokenResponse) {
  return typeof tokenResponse.created_at === "number" && typeof tokenResponse.expires_in === "number" && (tokenResponse.created_at + tokenResponse.expires_in) * 1e3 <= Date.now();
}
...
function loadLoginInfo(origin) {
  ...
  const obj = typeof str === "string" ? JSON.parse(str) : void 0;
  return isLoginInfo(obj) ? obj : void 0;
}
...
// Users/js/data/repos-llc/livewire-validator/validator-app/views/circular_progress_view.ts
var CIRCULAR_PROGRESS_CSS = css`
...
// Users/js/data/repos-llc/livewire-validator/validator-app/views/icons.ts
var INFO_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;
var WARNING_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M12 5.99L19.53 19H4.47L12 5.99M2.74 18c-.77 1.33.19 3 1.73 3h15.06c1.54 0 2.5-1.67 1.73-3L13.73 4.99c-.77-1.33-2.69-1.33-3.46 0L2.74 18zM11 11v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zm0 5h2v2h-2z"/></svg>`;
var ERROR_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M20.71 7.98L16.03 3.3c-.19-.19-.45-.3-.71-.3H8.68c-.26 0-.52.11-.7.29L3.29 7.98c-.18.18-.29.44-.29.7v6.63c0 .27.11.52.29.71l4.68 4.68c.19.19.45.3.71.3h6.63c.27 0 .52-.11.71-.29l4.68-4.68c.19-.19.29-.44.29-.71V8.68c.01-.26-.1-.52-.28-.7zM19 14.9L14.9 19H9.1L5 14.9V9.1L9.1 5h5.8L19 9.1v5.8z"/><circle cx="12" cy="16" r="1"/><path d="M12 7c-.55 0-1 .45-1 1v5c0 .55.45 1 1 1s1-.45 1-1V8c0-.55-.45-1-1-1z"/></svg>`;
var CHECK_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm3.88-11.71L10 14.17l-1.88-1.88c-.39-.39-1.02-.39-1.41 0-.39.39-.39 1.02 0 1.41l2.59 2.59c.39.39 1.02.39 1.41 0L17.3 9.7c.39-.39.39-1.02 0-1.41-.39-.39-1.03-.39-1.42 0z"/></svg>`;
var CHECKLIST_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><g><rect fill="none" height="24" width="24"/></g><g><g><path d="M5,5h2v1c0,1.1,0.9,2,2,2h6c1.1,0,2-0.9,2-2V5h2v5h2V5c0-1.1-0.9-2-2-2h-4.18C14.4,1.84,13.3,1,12,1S9.6,1.84,9.18,3H5 C3.9,3,3,3.9,3,5v14c0,1.1,0.9,2,2,2h6v-2H5V5z M12,3c0.55,0,1,0.45,1,1s-0.45,1-1,1s-1-0.45-1-1S11.45,3,12,3z"/><path d="M21.75,12.25c-0.41-0.41-1.09-0.41-1.5,0L15.51,17l-2.26-2.25c-0.41-0.41-1.08-0.41-1.5,0l0,0c-0.41,0.41-0.41,1.09,0,1.5 l3.05,3.04c0.39,0.39,1.02,0.39,1.41,0l5.53-5.54C22.16,13.34,22.16,12.66,21.75,12.25z"/></g></g></svg>`;
var SQUARE_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><g><rect fill="none" height="24" width="24"/></g><g><g><path d="M3,3v18h18V3H3z M19,19H5V5h14V19z"/></g></g></svg>`;
var PERSON_ICON = svg`<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v1c0 .55.45 1 1 1h14c.55 0 1-.45 1-1v-1c0-2.66-5.33-4-8-4z"/></svg>`;

// Users/js/data/repos-llc/livewire-validator/validator-app/views/util.ts
function externalizeAnchor(anchor) {
  anchor.target = "_blank";
  anchor.rel = "noreferrer noopener nofollow";
}

// Users/js/data/repos-llc/livewire-validator/validator-app/views/comments_view.ts
var COMMENTS_HTML = html`
...
`;
function initComments(document2, vm2) {
  const commentsDetails = document2.getElementById("comments-details");
  const commentsSubjectSpan = document2.getElementById("comments-subject");
  const commentsOutput = document2.getElementById("comments");
  return () => {
    const results = vm2.commentsResults;
    ...
    if (results !== _renderedResults) {
      renderComments(results, commentsOutput, vm2);
...
function renderNode(nodeId, threadcap, containerElement, level, vm2) {
  ...
  const commenter = comment ? threadcap.commenters[comment.attributedTo] : void 0;
  ...
  if (comment) {
    ...
    if (comment.url && threadcap.protocol === "activitypub") {
      ...
      replyAnchor.onclick = (e) => {
        e.preventDefault();
        toggleReplyBox(replyAnchor, replyFieldsetContainer, replyToUrl, vm2);
      };
  ...
  for (const reply of node.replies || []) {
    renderNode(reply, threadcap, containerElement, level + 1, vm2);
  }
}
...
function toggleReplyBox(anchor, fieldsetContainer, replyToUrl, vm2) {
  if (anchor.textContent?.startsWith("Reply")) {
    ...
    let newReplyUrl;
    const update = () => {
      const loggedIn = vm2.isLoggedIn(origin);
        ...
        if (w) {
          globalThis.onmessage = (e2) => {
            ...
            if (typeof data.origin === "string" && isOauthObtainTokenResponse(data.tokenResponse)) {
              vm2.acceptLogin(data.origin, data.tokenResponse);
              update();
              w.close();
            }
          };
    ...
    button.onclick = async (e) => {
      e.preventDefault();
      newReplyUrl = await vm2.sendReply(textarea.value.trim(), replyToUrl);
      sent = true;
    ...
    LitElement.render(void 0, fieldsetContainer);
...
// Users/js/data/repos-llc/livewire-validator/validator-app/views/form_view.ts
var FORM_HTML = html`
...
`;
function initForm(document2, vm2, staticData2, droppedFiles2) {
  const form = document2.getElementById("form");
  const textInput = document2.getElementById("text-input");
  const submitButton = document2.getElementById("submit");
  const versionSpan = document2.getElementById("version");
  const version = [staticData2.version, staticData2.pushId].map((v) => (v || "").trim()).filter((v) => v.length > 0).join(".");
  versionSpan.textContent = staticData2.version ? `v${version}` : "";
  document2.ondragover = (e) => e.preventDefault();
  document2.ondrop = async (e) => {
    e.preventDefault();
    try {
      const { name, text } = await getDroppedFileContents(e);
      if (!vm2.validating) {
        const fileUrl = `file://(dropped)/${name}`;
        droppedFiles2.set(new URL(fileUrl).toString(), text);
        textInput.value = fileUrl;
        vm2.startValidation(textInput.value, { validateComments: false, userAgent: navigator.userAgent });
      }
    } catch (e2) {
      console.log("Error in getDroppedFileText", e2);
    }
  };
  const { searchParams } = new URL(document2.URL);
  const validate2 = searchParams.get("validate") || void 0;
  const input = searchParams.get("input") || void 0;
  const nocomments = searchParams.has("nocomments");
  const startValidation = () => vm2.startValidation(textInput.value, { validateComments: !nocomments, userAgent: navigator.userAgent });
  form.onsubmit = (e) => {
    e.preventDefault();
    if (vm2.validating) {
      vm2.cancelValidation();
    } else {
      startValidation();
    }
  };
  if (validate2) {
    textInput.value = validate2;
  ...
  return () => {
    ...
    textInput.disabled = vm2.validating;
    textInput.readOnly = vm2.validating;
    submitButton.textContent = vm2.validating ? "Cancel" : "Validate";
...
// Users/js/data/repos-llc/livewire-validator/validator-app/views/messages_view.ts
var MESSAGES_HTML = html`
<output id="messages"></output>
`;
var MESSAGES_CSS = css`
...
`;
function initMessages(document2, vm2) {
  const messagesOutput = document2.getElementById("messages");
  return () => {
    LitElement.render(MESSAGE_HTML(vm2), messagesOutput);
  };
}
var MESSAGE_HTML = (vm2) => html`
    ${vm2.messages.filter(filterDuplicates()).map((message) => html`
...
var REFERENCE_HTML = (reference) => reference ? html`<a class="reference" href=${reference.href} target="_blank" rel="noreferrer noopener nofollow">[${reference.ruleset}]</a>` : void 0;
var ANCHOR_HTML = (url) => url ? html`<a href=${url} target="_blank" rel="noreferrer noopener nofollow" class="url">${url}</a>` : void 0;
function icon(type) {
...
function filterDuplicates() {
  const tagUrls = /* @__PURE__ */ new Set();
  return (message) => {
...
// Users/js/data/repos-llc/livewire-validator/validator-app/views/search_results_view.ts
var SEARCH_RESULTS_HTML = html`
<output id="search-results"></output>
`;
var SEARCH_RESULTS_CSS = css`
...
`;
function initSearchResults(document2, vm2) {
  const searchResultsOutput = document2.getElementById("search-results");
  return () => {
    LitElement.render(RESULTS_HTML(vm2), searchResultsOutput);
    searchResultsOutput.style.display = vm2.isSearch ? "block" : "none";
  };
}
var RESULTS_HTML = (vm2) => html`
    ${vm2.searchResults.map((result) => html`<div class="search-result" @click="${selectResult(vm2, result.url)}"><div class="icon">${IMAGE_HTML(result.artwork)}</div><div class="title">${result.title}</div><div class="author">${result.author}</div></div>`)}`;
var IMAGE_HTML = (artwork) => artwork ? html`<img src=${artwork}>` : SQUARE_ICON;
function selectResult(vm2, url) {
  return () => vm2.continueWith(url);
}

// Users/js/data/repos-llc/livewire-validator/validator-app/views/xml_view.ts
var XML_HTML = html`
...
`;
function initXml(document2, vm2) {
  const xmlOutput = document2.getElementById("xml");
  return () => {
    const xml = vm2.xml;
    if (xml !== _renderedXml) {
      renderXml(xml, xmlOutput, vm2.xmlSummaryText);
      _renderedXml = xml;
    }
  };
}
var MAX_ITEMS_TO_DISPLAY = 20;
var _renderedXml;
function renderXml(xml, xmlOutput, xmlSummaryText) {
  while (xmlOutput.firstChild) xmlOutput.removeChild(xmlOutput.firstChild);
  if (xml) renderNode2(xml, xmlOutput, 0, /* @__PURE__ */ new Set(), void 0, xmlSummaryText);
}
function renderNode2(node, containerElement, level, context, itemNumber, xmlSummaryText) {
  const { atts } = node;
  ...
  if (level === 0) {
    ...
    const spanClass = Qnames.PodcastIndex.NAMESPACES.includes(node.qname.namespaceUri || "") ? "podcast" : void 0;
  ...
  for (const [name, value] of Object.entries(node.child)) {
    ...
    let itemsNotShown = 0;
    for (const child of value) {
      if (name === "item" && itemNumber2 > MAX_ITEMS_TO_DISPLAY) {
        itemsNotShown++;
        continue;
      }
      renderNode2(child, details, level + 1, context, value.length > 1 ? itemNumber2 : void 0);
      childCount++;
      itemNumber2++;
    }
    if (itemsNotShown > 0) {
      const fakeNode = {
        ...
        atts: /* @__PURE__ */ new Map(),
        qname: { name: "" },
        attrsMap: {},
        child: {}
      };
      renderNode2(fakeNode, details, level - 1, context, void 0);
    }
  }
...
function renderTextPieces(element, ...pieces) {
  for (const piece of pieces) {
    ...
    const spanClass = typeof piece === "object" ? piece.spanClass : void 0;
...
// Users/js/data/repos-llc/livewire-validator/validator-app/validator_app.ts
var appModuleScript = document.getElementById("app-module-script");
...
function parseStaticData() {
  ...
  const flags = typeof data.flags === "string" ? data.flags : void 0;
  const debug = typeof data.debug === "object" ? data.debug : void 0;
  const pushId = typeof data.pushId === "string" ? data.pushId : void 0;
  return { version, flags, debug, pushId };
}
var staticData = parseStaticData();
var droppedFiles = /* @__PURE__ */ new Map();
var localFetcher = (url, headers) => {
...
var remoteFetcher = (url, headers) => fetch(`/f/${url.replaceAll(/[^a-zA-Z0-9.]+/g, "_")}`, { method: "POST", body: JSON.stringify({ url, headers }) });
var piSearchFetcher = (input, headers) => fetch(`/s`, { method: "POST", body: JSON.stringify({ input, headers }) });
var threadcapUserAgent = navigator.userAgent;
var vm = new ValidatorAppVM({ threadcapUserAgent, localFetcher, remoteFetcher, piSearchFetcher });
var updateForm = initForm(document, vm, staticData, droppedFiles);
var updateMessages = initMessages(document, vm);
var updateSearchResults = initSearchResults(document, vm);
var updateComments = initComments(document, vm);
var updateXml = initXml(document, vm);
vm.onChange = () => {
  updateForm();
  updateMessages();
  updateSearchResults();
